...
class TaskStatusManager:
    ...
    def set_pending(task: Task) -> None:
        if task.status not in (TaskStatus.NEW, TaskStatus.FAILED):
            raise ValueError(f"Can't set task {task.id} as pending: must be NEW or FAILED")
    ...
    def start(task: Task) -> None:
        if task.status != TaskStatus.PENDING:
            raise ValueError(f"Can't start {task.id}: must be PENDING")
    ...
    def complete(task: Task, result: str) -> None:
        if task.status != TaskStatus.IN_PROGRESS:
            raise ValueError(f"Can't complete {task.id}: must be IN_PROGRESS")
    ...
    def cancel(task: Task) -> None:
        if task.status not in (TaskStatus.NEW, TaskStatus.PENDING):
            raise ValueError(f"Can't cancel {task.id}: must be NEW or PENDING")
